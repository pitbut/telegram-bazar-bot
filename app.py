    ...
    if msg:
        ...
        if text == "/start":
            ...
            if user:
                ...
                tg("sendMessage", chat_id=chat_id, text=f"Ты уже в списке (код {user['family_code']}). Открывай кнопкой 🛒 у поля ввода.")
            else:
                tg(
                    "sendMessage",
                    chat_id=chat_id,
                    text="Привет! Создать новый список покупок или присоединиться к существующему по коду?",
                    reply_markup={
                        "inline_keyboard": [[
